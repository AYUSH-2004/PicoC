/*
 * PicoC Interpreter Project - Minimal C Interpreter with AST Visualization
 * Team: CodeCrusaders
 */
...
/* -------------------- Tokenizer (lexer) -------------------- */
typedef enum {
    TOK_INT, TOK_ID, TOK_PLUS, TOK_MINUS, TOK_MUL, TOK_DIV,
...
// Parse a single statement ending with optional semicolon
Node* parseStatement() {
    Node* expr = parseExpression();
...
// Parse multiple statements separated by semicolons until EOF
Node* parseStatements() {
    ...
    while (currentToken.type != TOK_EOF) {
        ...
        seqNode->data.binop.op = 0; // unused for sequence
        first = seqNode;
    }
    return first;
}
...
int evaluate(Node *node) {
    if (node->type == NODE_INT) {
        ...
        return evaluate(node->data.binop.right); // return last result
    }
    return 0;
}
...
int main() {
    ...
    if (currentToken.type == TOK_EOF) {
        ...
        printf("%d\n", result);  // print last expression's result
    } else {
        printf("Syntax Error!\n");
    }
